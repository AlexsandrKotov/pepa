...
import { login, bootstrapActivate, getBootstrapStatus, resetMyPassword, getMe, setStoredUser, getOIDCConfig, getOIDCLoginURL, getAzureConfig, getAzureLoginURL, getLDAPConfig, ldapLogin, getGoogleConfig, getGoogleLoginURL, getGitHubConfig, getGitHubLoginURL } from '@/lib/api';
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 relative bg-[#0d1520]">
        ...
        <div className="w-full max-w-sm relative z-10 page-animate">
          ...
          {phase === 'login' && (
            ...
            {(oidcEnabled || azureEnabled || googleEnabled || githubEnabled) && (
              <div className="mt-6">
                <div className="relative">
                  ...
                  {googleEnabled && (
                    <button
                      ...
                      onClick={async () => {
                        try {
                          setError('');
                          const { redirect_url } = await getGoogleLoginURL();
                          window.location.href = redirect_url;
                        } catch (err) {
                  ...
                  {githubEnabled && (
                    <button
                      ...
                      onClick={async () => {
                        try {
                          setError('');
                          const { redirect_url } = await getGitHubLoginURL();
                          window.location.href = redirect_url;
                        } catch (err) {
