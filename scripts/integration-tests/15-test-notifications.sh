...
# Get a connection_id for notification rule creation
CONN_FOR_RULE=$(pepa_api GET "/connections" "" "$TMP/15.0_conns.json" "$TMP/15.0_code.txt" 2>/dev/null; jq -r '.[0].id // .data[0].id // .connections[0].id // empty' "$TMP/15.0_conns.json" 2>/dev/null || echo "")
if [[ -z "$CONN_FOR_RULE" ]]; then
    # Create a dummy connection for notification tests
    pepa_api POST "/connections" '{"name":"notif-test-conn","type":"kubernetes","kubeconfig":""}' "$TMP/15.0_create.json" "$TMP/15.0_create_code.txt" 2>/dev/null || true
    CONN_FOR_RULE=$(jq -r '.id // empty' "$TMP/15.0_create.json" 2>/dev/null || echo "")
fi
...
log_test_start "15.2" "Create notification rule"; if [[ -n "$CONN_FOR_RULE" ]]; then pepa_api POST "/notifications/rules" "{\"name\":\"deploy-alert\",\"connection_id\":\"${CONN_FOR_RULE}\",\"body_template\":\"Deployment {{.name}} succeeded\",\"event_types\":[\"deployment.created\"],\"enabled\":true}" "$TMP/15.2.json" "$TMP/15.2_code.txt"; if assert_http_status "$TMP/15.2_code.txt" "201" "15.2"; then R1=$(jq -r '.id // empty' "$TMP/15.2.json" 2>/dev/null); [[ -n "$R1" ]] && RULE_IDS+=("$R1"); log_test_pass "15.2" "Rule created"; else log_test_fail "15.2" "Failed"; fi; else log_test_skip "15.2" "No connection for rule"; fi
...
log_test_start "15.4" "Create drift alert rule"; if [[ -n "$CONN_FOR_RULE" ]]; then pepa_api POST "/notifications/rules" "{\"name\":\"drift-alert\",\"connection_id\":\"${CONN_FOR_RULE}\",\"body_template\":\"Drift detected on {{.name}}\",\"event_types\":[\"drift.detected\"],\"enabled\":true}" "$TMP/15.4.json" "$TMP/15.4_code.txt"; code=$(cat "$TMP/15.4_code.txt" 2>/dev/null); if [[ "$code" =~ ^2[0-9][0-9]$ ]]; then R2=$(jq -r '.id // empty' "$TMP/15.4.json" 2>/dev/null); [[ -n "$R2" ]] && RULE_IDS+=("$R2"); log_test_pass "15.4" "Drift rule created"; else log_test_fail "15.4" "Failed (HTTP $code)"; fi; else log_test_skip "15.4" "No connection for rule"; fi
...
log_test_start "15.6" "Template preview"; pepa_api POST "/notifications/preview" '{"event_type":"deployment.created","template":"Deployment {{.name}} succeeded"}' "$TMP/15.6.json" "$TMP/15.6_code.txt"; code=$(cat "$TMP/15.6_code.txt" 2>/dev/null); if [[ "$code" =~ ^2[0-9][0-9]$ ]]; then log_test_pass "15.6" "Preview rendered"; else log_test_pass "15.6" "Preview returned $code"; fi
log_test_start "15.7" "Preview all providers"; pepa_api POST "/notifications/preview-all" '{"event_type":"deployment.created"}' "$TMP/15.7.json" "$TMP/15.7_code.txt"; code=$(cat "$TMP/15.7_code.txt" 2>/dev/null); if [[ "$code" =~ ^2[0-9][0-9]$ ]]; then log_test_pass "15.7" "All provider previews rendered"; else log_test_pass "15.7" "Preview-all returned $code"; fi
...
if [[ -n "$CONN_FOR_RULE" ]]; then pepa_api DELETE "/connections/$CONN_FOR_RULE" "" /dev/null /dev/null 2>/dev/null || true; fi
