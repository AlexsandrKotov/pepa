...
func dialSSH(ip, user string, signer ssh.Signer, timeout time.Duration) (*ssh.Client, error) {
	cfg := &ssh.ClientConfig{
		...
		HostKeyCallback: ssh.InsecureIgnoreHostKey(), //nolint:gosec // #nosec // freshly created container with a PEPA-injected key
...
func streamDockerImage(client *ssh.Client, image string) error {
	saveCmd := exec.Command("docker", "save", image) //nolint:gosec // #nosec // image name validated by caller context
...
func streamFolder(client *ssh.Client, folderPath, remoteDir string) error {
	...
	tarCmd := exec.Command("tar", "-czf", "-", "-C", abs, ".") //nolint:gosec // #nosec // path validated by caller
