...
func VerifyPluginBinary(binPath string, pubKey ed25519.PublicKey) error {
	...
	checksumRaw, err := os.ReadFile(checksumPath) //nolint:gosec // #nosec // G304: checksumPath is derived from validated plugin binary path
	if err != nil {
	...
	// 2. Read signature
	sig, err := os.ReadFile(sigPath) //nolint:gosec // #nosec // G304: sigPath is derived from validated plugin binary path
	if err != nil {
...
func VerifyPluginYAML(yamlPath string, pubKey ed25519.PublicKey) error {
	...
	checksumRaw, err := os.ReadFile(checksumPath) //nolint:gosec // #nosec // G304: checksumPath is derived from validated YAML path
	if err != nil {
	...
	sig, err := os.ReadFile(sigPath) //nolint:gosec // #nosec // G304: sigPath is derived from validated YAML path
	if err != nil {
...
func hashFile(path string) ([]byte, error) {
	...
	f, err := os.Open(path) //nolint:gosec // #nosec // G304: path is a validated plugin binary path
	if err != nil {
		return nil, err
	}
