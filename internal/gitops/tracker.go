...
func (t *DeployTracker) checkCIStatus(ctx context.Context, repo *Repo, commitSHA string) *DeployEvent {
	...
	// Use git ls-remote to check if commit is visible (basic connectivity)
	cmd := exec.CommandContext(ctx, "git", "ls-remote", repoURL, commitSHA)
	...
	// If the commit is found in remote, CI is likely running/passed
...
func (t *DeployTracker) checkFluxStatus(ctx context.Context, repo *Repo, commitSHA string) []DeployEvent {
	...
	cmd := exec.CommandContext(ctx, "kubectl", "get", "helmreleases", "--all-namespaces",
