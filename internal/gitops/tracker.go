...
func (t *DeployTracker) checkCIStatus(ctx context.Context, repo *Repo, commitSHA string) *DeployEvent {
	...
	cmd := exec.CommandContext(ctx, "git", "ls-remote", repoURL, commitSHA) // #nosec //nolint:gosec // G204: git ls-remote with validated args
...
func (t *DeployTracker) checkFluxStatus(ctx context.Context, repo *Repo, commitSHA string) []DeployEvent {
	...
	cmd := exec.CommandContext(ctx, "kubectl", "get", "helmreleases", "--all-namespaces", // #nosec //nolint:gosec // G204: kubectl with static args
