...
func (e *Editor) ApplyEdit(ctx context.Context, repo *Repo, req *EditRequest) (*CommitResult, error) {
	...
	cmd := exec.CommandContext(ctx, "git", cloneArgs...) //nolint:gosec // #nosec // G204: git clone with validated args
	...
	for _, cfg := range []string{"user.email=" + gitEmail, "user.name=" + gitName} {
		cmd = exec.CommandContext(ctx, "git", "-C", tmpDir, "config", strings.SplitN(cfg, "=", 2)[0], strings.SplitN(cfg, "=", 2)[1]) //nolint:gosec // #nosec // G204: git config with validated args
		if out, err := cmd.CombinedOutput(); err != nil {
	...
	originalContent, err := os.ReadFile(targetPath) //nolint:gosec // #nosec // G304: targetPath is validated against path traversal
	if err != nil {
	...
	// Stage the change
	cmd = exec.CommandContext(ctx, "git", "-C", tmpDir, "add", cleanPath) //nolint:gosec // #nosec // G204: git add with validated path
	if out, err := cmd.CombinedOutput(); err != nil {
	...
	cmd = exec.CommandContext(ctx, "git", "-C", tmpDir, "commit", "-m", commitMsg) //nolint:gosec // #nosec // G204: git commit in controlled temp dir
	if out, err := cmd.CombinedOutput(); err != nil {
		return nil, fmt.Errorf("git commit: %s: %w", string(out), err)
	}

	// Try to push directly
	pushCmd := exec.CommandContext(ctx, "git", "-C", tmpDir, "push", "origin", branch) //nolint:gosec // #nosec // G204: git push in controlled temp dir
	...
	cmd = exec.CommandContext(ctx, "git", "-C", tmpDir, "branch", "-m", featureBranch) //nolint:gosec // #nosec // G204: git branch in controlled temp dir
	if out, err := cmd.CombinedOutput(); err != nil {
		return nil, fmt.Errorf("git branch rename: %s: %w", string(out), err)
	}

	pushCmd = exec.CommandContext(ctx, "git", "-C", tmpDir, "push", "origin", featureBranch) //nolint:gosec // #nosec // G204: git push in controlled temp dir
	pushCmd.Env = append(os.Environ(), "GIT_TERMINAL_PROMPT=0")
...
func getHeadSHA(ctx context.Context, dir string) string {
	cmd := exec.CommandContext(ctx, "git", "-C", dir, "rev-parse", "HEAD") //nolint:gosec // #nosec // G204: git rev-parse in controlled dir
	out, err := cmd.Output()
	if err != nil {
		return ""
	}
