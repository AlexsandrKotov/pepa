...
func setupMockVault(t *testing.T) (*httptest.Server, *Client) {
	t.Helper()
	...
	mux.HandleFunc("/v1/secret/data/test/app", func(w http.ResponseWriter, r *http.Request) {
		switch r.Method {
		case "GET":
			json.NewEncoder(w).Encode(map[string]interface{}{
			...
			json.NewEncoder(w).Encode(map[string]interface{}{
				"data": map[string]interface{}{
					"metadata": map[string]interface{}{
						"version":      3,
						"created_time": "2024-01-01T00:00:00Z",
						"updated_time": "2024-07-01T00:00:00Z",
						"destroyed":    false,
					},
				},
			})
	...
	mux.HandleFunc("/v1/secret/data/test/missing", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(404)
		w.Write([]byte(`{"errors":[]}`))
	})

	mux.HandleFunc("/v1/secret/metadata/test", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("list") == "true" {
			json.NewEncoder(w).Encode(map[string]interface{}{
				"data": map[string]interface{}{
					"keys": []string{"app", "db", "api"},
				},
			})
		}
	})
	...
	mux.HandleFunc("/v1/secret/metadata", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]interface{}{
			"data": map[string]interface{}{
				"keys": []string{"test", "prod"},
			},
		})
	})

	mux.HandleFunc("/v1/secret/metadata/test/delete-me", func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "DELETE" {
			w.WriteHeader(204)
			return
		}
	...
	if err != nil {
		// The test server uses 127.0.0.1 which is blocked by SSRF protection.
		// We need to bypass that for testing. Create client directly.
		t.Fatalf("NewClient with test server: %v", err)
	}
