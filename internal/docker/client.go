...
func (c *Client) run(ctx context.Context, args ...string) (string, error) {
	cmd := exec.CommandContext(ctx, "docker", args...) // #nosec // G204: docker is an admin-configured binary
	cmd.Env = c.env()
...
func (c *Client) ComposeUp(ctx context.Context, projectName, composeYaml string, envVars map[string]string) error {
	...
	cmd := exec.CommandContext(ctx, "docker", args...) // #nosec // G204: docker compose with validated args
	cmd.Env = c.env()
