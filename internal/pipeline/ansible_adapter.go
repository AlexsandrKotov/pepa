...
func (a *AnsibleAdapter) ResolveSchema(ctx context.Context, raw json.RawMessage) (*ParameterSchema, error) {
	...
	data, err := os.ReadFile(absPlaybook) //nolint:gosec // #nosec // G304: absPlaybook is validated to be within workDir
	if err != nil {
...
func (a *AnsibleAdapter) Trigger(ctx context.Context, raw json.RawMessage, params map[string]any) (*TriggerResult, error) {
	...
	cmd := exec.CommandContext(runCtx, "ansible-playbook", args...) //nolint:gosec // #nosec // G204: ansible-playbook is an admin-configured binary
	cmd.Dir = workDir
	cmd.Stdout = logBuf
	cmd.Stderr = logBuf
...
func (a *AnsibleAdapter) Plan(ctx context.Context, raw json.RawMessage, params map[string]any) (*PlanResult, error) {
	...
	cmd := exec.CommandContext(ctx, "ansible-playbook", args...) //nolint:gosec // #nosec // G204: ansible-playbook is an admin-configured binary
	cmd.Dir = workDir
	cmd.Env = append(os.Environ(),
		"ANSIBLE_NOCOLOR=false",
		"ANSIBLE_FORCE_COLOR=true",
...
func parseInventoryFile(path string) []StateResource {
	data, err := os.ReadFile(path) //nolint:gosec // #nosec // G304: path is within validated workDir
	if err != nil {
		return nil
	}
...
func (a *AnsibleAdapter) Inspect(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
	...
	_ = filepath.Walk(workDir, func(path string, info os.FileInfo, walkErr error) error {
		...
		data, readErr := os.ReadFile(path) //nolint:gosec // #nosec // G304: path is within validated workDir
		if readErr != nil {
			return nil
		}
		var plays []map[string]interface{}
	...
	if entries, err := os.ReadDir(rolesDir); err == nil {
		for _, entry := range entries {
			...
			if taskEntries, err := os.ReadDir(tasksDir); err == nil {
				for _, te := range taskEntries {
					if !te.IsDir() && (filepath.Ext(te.Name()) == ".yml" || filepath.Ext(te.Name()) == ".yaml") {
						data, readErr := os.ReadFile(filepath.Join(tasksDir, te.Name())) //nolint:gosec // #nosec // G304: validated path
						if readErr == nil {
							var tasks []interface{}
			...
			if metaData, err := os.ReadFile(metaPath); err == nil { //nolint:gosec // #nosec // G304: validated path
...
func gitClone(ctx context.Context, repoURL, token, destDir string) error {
	...
	cmd := exec.CommandContext(ctx, "git", "clone", "--depth", "1", cloneURL, destDir) //nolint:gosec // #nosec // G204: git clone with validated args
