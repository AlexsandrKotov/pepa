...
func selectWorkspace(ctx context.Context, bin, workDir, workspace string) error {
	...
	cmd := exec.CommandContext(ctx, bin, "workspace", "select", workspace) // #nosec // G204: bin and workspace from pipeline config
	cmd.Dir = workDir
	if err := cmd.Run(); err == nil {
		return nil
	}
	// If select failed, try to create it
	cmd = exec.CommandContext(ctx, bin, "workspace", "new", workspace) // #nosec // G204: bin and workspace from pipeline config
	cmd.Dir = workDir
	return cmd.Run()
}
...
func (a *TerraformAdapter) ResolveSchema(ctx context.Context, raw json.RawMessage) (*ParameterSchema, error) {
	...
	for _, tfFile := range tfFiles {
		data, err := os.ReadFile(tfFile) // #nosec // G304: tfFile is from a validated directory listing
		if err != nil {
			continue
		}
...
func detectBackendType(workDir string) string {
	...
	for _, f := range files {
		data, err := os.ReadFile(f) // #nosec // G304: validated directory listing
		if err != nil {
			continue
		}
...
func sanitizeBackendVars(workDir string) {
	...
	for _, f := range files {
		...
		if !bytes.Equal(data, modified) {
			_ = os.WriteFile(f, modified, 0600) // #nosec // G703: f from validated directory listing
		}
	}
}
...
func (a *TerraformAdapter) Trigger(ctx context.Context, raw json.RawMessage, params map[string]any) (*TriggerResult, error) {
	...
	// Run IaC init
	bin := iacBinary()
	initCmd := exec.CommandContext(runCtx, bin, "init", "-input=false") // #nosec // G204: bin from pipeline config
	initCmd.Dir = workDir
	...
	tfCmd := exec.CommandContext(runCtx, bin, args...) // #nosec // G204: bin and args from pipeline config
	tfCmd.Dir = workDir
	tfCmd.Stdout = logBuf
	tfCmd.Stderr = logBuf
...
func (a *TerraformAdapter) Plan(ctx context.Context, raw json.RawMessage, params map[string]any) (*PlanResult, error) {
	...
	// IaC init
	bin := iacBinary()
	initCmd := exec.CommandContext(ctx, bin, "init", "-input=false", "-no-color") // #nosec // G204: bin from pipeline config
	initCmd.Dir = workDir
	...
	planCmd := exec.CommandContext(ctx, bin, planArgs...) // #nosec // G204: IaC CLI is an expected subprocess
	planCmd.Dir = workDir
	...
	showCmd := exec.CommandContext(ctx, bin, "show", "-json", planFile) // #nosec // G204: IaC CLI is an expected subprocess
	showCmd.Dir = workDir
...
func (a *TerraformAdapter) State(ctx context.Context, raw json.RawMessage, params map[string]any) (*StateResult, error) {
	...
	// IaC show -json
	showCmd := exec.CommandContext(ctx, bin, "show", "-json") // #nosec // G204: bin from pipeline config
	showCmd.Dir = workDir
...
func (a *TerraformAdapter) Inspect(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
	...
	for _, tfFile := range tfFiles {
		data, readErr := os.ReadFile(tfFile) // #nosec // G304: validated path
		if readErr != nil {
			continue
		}
	...
	bin := iacBinary()
	wsCmd := exec.CommandContext(ctx, bin, "workspace", "list") // #nosec // G204: bin from pipeline config
	wsCmd.Dir = workDir
...
func copyDir(src, dst string) error {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			if err := os.MkdirAll(dstPath, 0750); err != nil { // #nosec // G301: standard directory copy
				return err
			}
			...
			data, err := os.ReadFile(srcPath) // #nosec // G304: srcPath from copyDir input
			if err != nil {
				return err
			}
			if err := os.WriteFile(dstPath, data, 0600); err != nil { // #nosec // G306: standard file copy
				return err
			}
		}
	}
