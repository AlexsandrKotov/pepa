...
func detectBackendType(workDir string) string {
	...
	for _, f := range files {
		data, err := os.ReadFile(f) //nolint:gosec // G304: validated directory listing
		if err != nil {
			continue
		}
...
func (a *TerraformAdapter) Plan(ctx context.Context, raw json.RawMessage, params map[string]any) (*PlanResult, error) {
	...
	planCmd := exec.CommandContext(ctx, bin, planArgs...) //nolint:gosec // G204: IaC CLI is an expected subprocess
	planCmd.Dir = workDir
	...
	showCmd := exec.CommandContext(ctx, bin, "show", "-json", planFile) //nolint:gosec // G204: IaC CLI is an expected subprocess
	showCmd.Dir = workDir
