...
func main() {
	...
	if rawToken, err := rest.SeedBootstrapToken(bootstrapDeps); err != nil {
		...
		if tokenPath == "" {
			tokenPath = "/var/run/pepa/bootstrap_token.txt" // #nosec //nolint:gosec // G101: not a credential, just a default file path
		}
		if writeErr := os.MkdirAll(filepath.Dir(tokenPath), 0700); writeErr != nil { // #nosec //nolint:gosec // G703: tokenPath is admin-controlled (env var or hardcoded default)
			slog.Warn("failed to create token directory", "error", writeErr)
		}
		if writeErr := os.WriteFile(tokenPath, []byte(rawToken+"\n"), 0600); writeErr != nil { // #nosec //nolint:gosec // G703: tokenPath is admin-controlled (env var or hardcoded default)
